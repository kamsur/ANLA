...
# from krylov_musterlsg import cg, gmres
...
def magic(A, b, tol=1e-12):
    ...
    AtPiPib = A.T @ solve_triangular(P.T @ P, b)
    AtPiPiA = A.T @ solve_triangular(P.T @ P, A)
    return cg(AtPiPiA, AtPiPib, tol=tol)
...
if __name__ == "__main__":
    settings = [0, 0.1, 1]
    fig, ax = pl.subplots(len(settings), 1)
    fig.tight_layout()
    for s, ω in enumerate(settings):
        A = create_A(30, ω)
